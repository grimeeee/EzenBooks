...
const CartPage = () => {
  ...
  localStorage.setItem("total", cart.totalItems)

  
  var title = cart.items.map((elem) => {
    ...
    if (localStorage.getItem('username') === null) {
      ...
      for (var i = 0; i < cart.items.length; i++) {
        {user.slice().map((user) => {
          ...
          let array2 = [];
          array2.push(cart.items[i].id)
  
        formData.append('books_book_num', array2);
        formData.append('user_id', user.user.id);
        formData.append('price', localStorage.getItem('cartTotal'));
        formData.append('book_count', cart.totalItems);
